...
description: Core software engineering principles (SOLID, DRY, YAGNI, KISS) and refactoring catalog. Integrates strictly with Native Workflow Memory for tracking technical debt. Use this skill when reviewing code, planning architecture, evaluating technical debt, or executing refactoring.
---
...
## 1. Document & Workflow Sync (Technical Debt)

When these standards are applied or violated, findings must be anchored using **standard workspace file tools**:
1. **Execution Artifact (`agent-output/`)**: Save full technical analyses, ADRs, or code-review findings.
2. **Workflow Graph (`workflows/`)**: Major architectural refactoring decisions must be logged in a `WF-*` node following the strict **10-Line Rule** using standard native markdown links.
...
Before concluding your turn after a review or refactoring session, you MUST align the workflow using **standard workspace file tools**:

1. **The Artifact (`agent-output/`)**: 
   * Write your refactoring plans or review findings to the appropriate directory using `edit/createFile` or `edit/editFiles`.
   * If a refactoring or review document reaches a terminal state, use the close script via terminal: `sh .github/scripts/close_document.sh <path-to-your-file.md> "Resolved"`.
