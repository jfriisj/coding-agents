...
description: Comprehensive security methodology, OWASP Top 10, and templates. Uses Progressive Disclosure to lazy-load language-specific vulnerabilities. Integrates strictly with Native Workflow Memory. Use this skill for security audits, code reviews, and threat modeling.
---
...
## 1. Document & Workflow Sync (Security)

Every security audit or threat model must be synchronized across two structural layers:
1. **Execution Artifact (`agent-output/security/`)**: The detailed assessment using the template below.
2. **Workflow Graph (`workflows/`)**: The `WF-*` node linking your security verdict to the Plan or Implementation using standard native markdown links.

---

## 2. Tooling Contract (Native Commands & Search)

**CRITICAL RULE**: Do NOT use terminal bash scripts. You MUST use structured native tools:
...
### Dependency Checks
Use `read/readFile` to read `package.json`, `requirements.txt`, or `go.mod` to identify outdated dependencies.

---

## 3. Language-Specific Deep Dives (Progressive Disclosure)

To save context tokens, this skill uses lazy loading for deep language specifics. Once you identify the primary language of the repository, you MUST use `read/readFile` to load the corresponding reference:
...
## 7. Document & Workflow Sync (The Handoff Protocol)

Before concluding your turn, you MUST align the workflow using **standard workspace file tools**:

1. **The Artifact (`agent-output/`)**: 
   * Save the filled document template using `edit/createFile` or `edit/editFiles`.
   * If a security document reaches a terminal state (e.g., `Closed` or `Approved`), use the close script via terminal: `sh .github/scripts/close_document.sh <path-to-your-file.md> "Closed"`.

2. **The Memory (Workflow Graph)**:
   * Use native file tools to create or update your `WF-<concrete-id>-<slug>.md` node.
   * Strictly follow the **10-Line Rule** (frontmatter `type: Security`, `parent: "workflows/WF-...md"`, and max 3-bullet summary).
